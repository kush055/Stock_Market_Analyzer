...
def get_nifty_option_chain():
    ...
    try:
        ...
        year = today.year
        month = today.month
        expiry_date = get_expiry_date(year, month)  # Get the expiry date for this month
        
        if not expiry_date:
            print(f"No expiry date found for Nifty options in {month}-{year}.")
            return None
        
        # Fetch option chain data for Nifty (Call options example)
        option_chain = get_history(symbol="NIFTY", index=True, start="2024-12-01", end="2024-12-31", option_type="CE", strike_price=20000)
        return option_chain
    except Exception as e:
