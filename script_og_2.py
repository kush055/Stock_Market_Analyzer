...
def get_nifty_option_chain():
    """Fetch the option chain for Nifty 50 from NSE."""
    nse = Nse()
    try:
...
def display_option_chain(option_chain):
    ...
    if option_chain:
        ...
        print("-" * 50)
        for data in option_chain['records']['data'][:10]:  # Displaying only the first 10 entries
            strike_price = data['strikePrice']
            ce_oi = data['CE']['openInterest'] if 'CE' in data else "NA"
            ce_ltp = data['CE']['lastPrice'] if 'CE' in data else "NA"
            pe_oi = data['PE']['openInterest'] if 'PE' in data else "NA"
            pe_ltp = data['PE']['lastPrice'] if 'PE' in data else "NA"
